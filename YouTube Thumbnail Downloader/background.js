chrome.browserAction.onClicked.addListener(function() {
	chrome.tabs.query({ currentWindow: true, active: true }, function(tabs) {
		if (tabs[0].url.includes('youtube.com/watch?v=')) {
			let filename  = tabs[0].title.replace(/ - YouTube$/, '') + '.jpg';
			...
			chrome.downloads.download({ url: url1, filename: filename, saveAs: true }, function(downloadId) {
	                        if (typeof downloadId === 'number') { return; }
				chrome.downloads.download({ url: url2, filename: filename, saveAs: true });
			});
