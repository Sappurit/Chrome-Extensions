chrome.browserAction.onClicked.addListener(function(activeTab) {
	chrome.tabs.query({ currentWindow: true, active: true }, function(tabs) {
		if (tabs[0].url.indexOf("www.youtube.com/watch?v=") == -1) {
			...
			chrome.downloads.download({ url: url1, filename: youTubeId + ".jpg", saveAs: true }, function(downloadId) {
	                        if (typeof downloadId === "integer")
	                        {
					return;
	                        }
			});

			chrome.downloads.download({ url: url2, filename: youTubeId + ".jpg", saveAs: true }, function(downloadId) {
	                        if (typeof downloadId === "integer")
	                        {
					return;
	                        }
